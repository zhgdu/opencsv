...
Because opencsv is multithreaded by default then the getCapturedExceptions() method in CsvToBean and StatefulBeanToCsv can potentially return more exceptions than expected.
...
So if the actual contents of the exceptions is important then you are better off using an single threaded iterator and queueing the exceptions yourself than a multithreaded solution.

Just beware that the more threads your system has allocated and the heavier load on the system can increase the above mentioned issues.
